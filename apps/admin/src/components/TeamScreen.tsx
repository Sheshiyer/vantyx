...
export function TeamScreen({ selfEmail, onClose }: { selfEmail: string; onClose: () => void }) {
  ...
  async function invite(e: FormEvent) {
    ...
    try {
      ...
      setToast(res.emailed ? `Invite emailed to ${res.email}.` : `Invite created — copy the link below.`);
      if (!res.emailed) setToast(`Invite link for ${res.email}: ${res.activateUrl}`);
      setEmail("");
      await load();
    } catch (e) {
  ...
  async function patch(m: TeamMember, p: { role?: Role; status?: "active" | "disabled" }) {
    ...
    setToast("");
    try {
      await updateTeammate(m.email, p);
      await load();
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4 backdrop-blur" onClick={onClose}>
      ...
      >
        ...
        {(error || toast) && (
          <p className={cn("px-5 pt-3 text-xs break-all", error ? "text-rose-300" : "text-emerald-300")}>
            {error || toast}
          </p>
        )}
            ...
            members.map((m) => {
              ...
              return (
                <div key={m.email} className="flex flex-wrap items-center gap-2 rounded-lg px-3 py-2 hover:bg-white/5">
                  ...
                  {isSelf && <span className="text-[11px] text-white/30">(you)</span>}
